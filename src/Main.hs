...
importablePatterns :: Pattern
importablePatterns = "importable/*.md" .||. "importable/*.html"
...
main = hakyll $ do
  ...
  -- | Importable: The posts in a format that's importable by other sites
  match importablePatterns $ do
    route   $ setExtension "html"
    compile $ pandocCompiler
      -- Create snapshots of the raw post, before any templates are applied. We can get this
      -- later by loading the snapshot from `raw-post-content`.
