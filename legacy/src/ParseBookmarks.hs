...
generateReadingList month year links =
  let haskellLinks = filterDomain "reddit.com/r/haskell" links
      hackerNewsLinks = filterDomain "news.ycombinator.com" links
      otherLinks = length links - length haskellLinks - length hackerNewsLinks
  in HaveRead
     { hrTotal = length links
...
filterDomain domain = filter (hasDomain . bmLink)
  where
    hasDomain :: Text -> Bool
    hasDomain l =
      isPrefixOf (append "http://" domain) l ||
